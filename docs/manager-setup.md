...
В VS Code слева — колонка значков. Нажмите **Extensions**, в поиске наберите `ChatGPT`, у расширения **ChatGPT – Codex** нажмите **Install**.

## 4. Войдите в Codex

В той же колонке появится значок Codex. Нажмите его, затем **Sign in** — откроется браузер, войдите рабочей учётной записью и вернитесь в VS Code.
...
Когда спросят, куда сохранить, выберите **Documents**. Когда предложат открыть — **Open**.

Папка окажется здесь, и это единственное место, где она должна быть:

- Windows — `C:\Users\<ваше имя>\Documents\qmedia-ads-agent`
- macOS — `Documents/qmedia-ads-agent`

Если потеряете её потом: **File → Open Recent**.

## 6. Разрешите Codex работать с папкой

Codex спросит, доверяете ли вы этой папке. Ответьте да.

## 7. Включите автообновление

Откройте настройки Codex, раздел **Hooks**. Там будет одна строка с пометкой **New hook** — нажмите **Trust**.

Это разрешает инструменту обновляться самому. Пропустите шаг — обновления просто не будут приходить, и вы об этом не узнаете.

## 8. Подключите рекламные кабинеты

В панели Codex откройте меню команд и выберите **Configure MCP servers**. В списке три сервера: `google-ads`, `lidfly`, `meta`. У каждого нажмите вход и пройдите его в браузере.

- `google-ads` — вашим рабочим Google-аккаунтом, тем самым, который внесли в Test users.
- `lidfly` — учётной записью LidFly.
- `meta` — обычный вход в Facebook рабочим аккаунтом.

Готово, когда все три перестали быть помеченными как требующие входа.
